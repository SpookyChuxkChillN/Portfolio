...
def delete_stock(stock_list):
    ...
    if found == True:
        ...
        print ("Error, Stock not found in list")
    _=input ("Press Enter to continue ***")

    
# List stocks being tracked
def list_stocks(stock_list):
    print ("Stock List Menu")
    print ("{:<20} {:<20} {:<20}".format('SYMBOL', 'NAME', 'SHARE'))
    print ("=" *54)
    for stock in stock_list:
        print ("{:<20} {:<20} {:<20}".format(stock.symbol, stock.name, stock.shares))
    _=input ("Press Enter to continue ***")
...
def display_chart(stock_list):
    print ("Display Stock Menu")
    print("Stock List: [", end="")
    for stock in stock_list:
        print (stock.symbol, " ", end="")
    print ("]")
    symbol = input ("Which stock do you want to display?: ").upper()
    found = False
    for stock in stock_list:
        if stock.symbol == symbol:
            found = True
...
def display_report(stock_list):
    ...
    for stock in stock_list:
        ...
        price_total = 0
        volume_total = 0
        lowPrice = 9999999999.99
        highPrice = 0.0
        lowVolume = 9999999999.99
        highVolume = 0
    ...
    while True:
        ...
        if option == "1":
           ...
           add_stock_data(stock_list) 
            ...
            print("Error: Please select an correct option.")
